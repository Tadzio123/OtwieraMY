			...
			>
				...
				{coordinates.map((place) => (
					<Marker
						...
						eventHandlers={{
							click: () => {
								setActiveMarkerFunc(place.id);
								map.setView([place.coordinateX, place.coordinateY], 10);
							},
						}}
					/>
				))}
				{activeMarkerData && (
					...
					>
						...
						<Typography component="p" type="font-sm-light">
							{activeMarkerData.street} {activeMarkerData.number}
							{', '}
							{activeMarkerData.postalCode} {activeMarkerData.city}
						</Typography>
					</StyledPopup>
				)}
			</StyledMapContainer>
